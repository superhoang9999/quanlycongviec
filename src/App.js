                  ...
                  <tbody className="divide-y divide-gray-100 text-xs md:text-sm">
                    {rolesList.map(r => (
                      <tr key={r.id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4 text-center"><div className="flex items-center justify-center space-x-2"><button onClick={() => handleOpenRoleForm(r)} disabled={r.name === 'Admin'} className={`p-2 rounded-lg ${r.name === 'Admin' ? 'text-gray-300' : 'text-blue-600 hover:bg-blue-50'}`}><Edit className="w-4 h-4"/></button><button onClick={() => handleDeleteRole(r.id, r.name)} disabled={r.name === 'Admin'} className={`p-2 rounded-lg ${r.name === 'Admin' ? 'text-gray-300' : 'text-red-600 hover:bg-red-50'}`}><Trash2 className="w-4 h-4"/></button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
