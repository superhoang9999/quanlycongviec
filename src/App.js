...
export default function App() {
  ...
  // State Chế độ xem (Cá nhân / Nhóm) & Lọc KPI
  ...
  // Refs lưu trữ cho Thông báo ngầm
  const prevTaskIdsRef = useRef(new Set());
  // Ref để lưu ngày cuối cùng đã thực hiện gửi thông báo 15h
  ...
  // --- BỘ LỌC THEO NHÓM, CÁ NHÂN VÀ KPI ---
  ...
  const handleChangePassword = async (e) => {
    ...
    if (sheetUrl) {
      try {
        await fetch(sheetUrl, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'update', sheetName: 'Users', ...updatedUser }) });
      ...
      if (result.status === 'success') {
        if (result.tasks) {
          ...
          if (fetchedTasks.length > 0) {
             ...
             const now = new Date();
             const todayString = now.toDateString(); // VD: "Sat Mar 07 2026"
             ...
             if (Notification.permission === "granted" && currentUserNow) {
               ...
               fetchedTasks.forEach(task => {
                 ...
                 // 1. THÔNG BÁO VIÊC MỚI (Báo ngay lập tức khi phát hiện ID mới)
                 ...
                 // 2. THÔNG BÁO NHẮC DEADLINE (Chỉ chạy vào 15h và mỗi ngày báo 1 lần)
                 if (isMine && task.tienDo !== 'Hoàn thành' && task.thoiHan) {
                   // Kiểm tra khung giờ 15h và chưa gửi tin của "ngày hôm nay"
               ...
               // Sau khi duyệt hết tất cả task ở khung giờ 15h, đánh dấu đã hoàn thành nhắc nhở của ngày hôm nay
               if (now.getHours() === 15) {
                 lastKpiNotifDateRef.current = todayString;
               }
             }
          ...
          <form onSubmit={handleLogin} className="p-8 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required className="pl-10 w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" placeholder="Nhập tên tài khoản..." value={loginForm.username} onChange={(e) => setLoginForm({...loginForm, username: e.target.value})} />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" required className="pl-10 w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" placeholder="••••••••" value={loginForm.password} onChange={(e) => setLoginForm({...loginForm, password: e.target.value})} />
              </div>
            </div>
    ...
    <div className="flex h-screen bg-gray-50 text-gray-800 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-gray-50/50">
        <div className="md:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm z-10 flex-shrink-0">
          ...
          <button onClick={() => setIsMobileMenuOpen(true)} className="p-1.5 bg-gray-100 rounded-lg text-gray-600 hover:bg-gray-200 active:scale-95 transition-transform"><Menu className="w-6 h-6" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
            ...
            <div className="pb-8">
              <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 md:mb-8 gap-4">
                <div className="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
                  <h2 className="text-xl md:text-2xl font-bold text-gray-800 hidden md:block mr-2">Tổng quan</h2>
                  <div className="flex flex-wrap gap-2 md:gap-3 w-full md:w-auto">
                    <div className="flex bg-gray-200 p-1 rounded-lg flex-1 md:flex-none">
                      <button onClick={() => setViewMode('personal')} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${viewMode === 'personal' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>Của tôi</button>
                      <button onClick={() => setViewMode('group')} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${viewMode === 'group' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>{isAdmin ? 'Hệ thống' : 'Của Nhóm'}</button>
                    </div>
                    <div className="flex bg-gray-200 p-1 rounded-lg flex-1 md:flex-none">
                      <button onClick={() => setIsKpiMode(false)} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${!isKpiMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>Tất cả</button>
                      <button onClick={() => setIsKpiMode(true)} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors flex items-center justify-center ${isKpiMode ? 'bg-white text-rose-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}><AlertCircle className="w-3.5 h-3.5 mr-1" /> KPI</button>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'tasks' && (
            <div className="h-full flex flex-col pb-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div className="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
                  <h2 className="text-xl md:text-2xl font-bold text-gray-800 hidden md:block mr-2">Công việc</h2>
                  <div className="flex flex-wrap gap-2 md:gap-3 w-full md:w-auto">
                    <div className="flex bg-gray-200 p-1 rounded-lg flex-1 md:flex-none"><button onClick={() => setViewMode('personal')} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${viewMode === 'personal' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>Của tôi</button><button onClick={() => setViewMode('group')} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${viewMode === 'group' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>{isAdmin ? 'Hệ thống' : 'Nhóm'}</button></div>
                    <div className="flex bg-gray-200 p-1 rounded-lg flex-1 md:flex-none"><button onClick={() => setIsKpiMode(false)} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors ${!isKpiMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}>Tất cả</button><button onClick={() => setIsKpiMode(true)} className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs md:text-sm font-semibold transition-colors flex items-center justify-center ${isKpiMode ? 'bg-white text-rose-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}><AlertCircle className="w-3.5 h-3.5 mr-1" /> KPI</button></div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex-1 overflow-hidden flex flex-col">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead><tr className="bg-gray-50 border-b border-gray-200 text-xs md:text-sm font-semibold text-gray-600"><th className="p-4 whitespace-nowrap">Nội dung công việc</th><th className="p-4 whitespace-nowrap">Phân loại</th>{isAdmin && <th className="p-4 whitespace-nowrap">Nhóm</th>}<th className="p-4 whitespace-nowrap">Thời hạn</th><th className="p-4 whitespace-nowrap">Phụ trách</th><th className="p-4 whitespace-nowrap">Tiến độ</th><th className="p-4 text-center whitespace-nowrap">Tỷ lệ</th><th className="p-4 text-center whitespace-nowrap sticky right-0 bg-gray-50 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.05)]">Thao tác</th></tr></thead>
                    <tbody className="divide-y divide-gray-100 text-xs md:text-sm">{filteredTasks.length > 0 ? filteredTasks.map((task) => (<tr key={task.id} className="hover:bg-blue-50 transition-colors cursor-pointer group" onClick={() => setViewingTask(task)}><td className="p-4 max-w-[200px] md:max-w-xs"><div className="font-bold text-gray-800 line-clamp-2" title={task.noiDung}>{task.noiDung}</div><div className="text-[11px] md:text-xs text-gray-500 line-clamp-2 mt-1" title={task.chiTiet}>{task.chiTiet}</div></td><td className="p-4"><span className={`inline-flex items-center px-2 py-1 rounded border whitespace-nowrap shadow-sm text-[10px] md:text-xs font-bold ${CATEGORY_COLORS[task.phanLoai] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>{task.phanLoai}</span></td>{isAdmin && (<td className="p-4"><span className="bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-1 rounded text-[10px] md:text-xs font-bold whitespace-nowrap">{task.nhom}</span></td>)}<td className="p-4 text-gray-600 font-medium whitespace-nowrap">{task.thoiHan ? (!isNaN(new Date(task.thoiHan).getTime()) ? new Date(task.thoiHan).toLocaleDateString('vi-VN') : task.thoiHan) : '---'}</td><td className="p-4 text-gray-800 font-medium"><div className="flex flex-wrap gap-1">{task.nguoiPhuTrach.split(',').map((name, i) => (<span key={i} className="inline-block bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[10px] md:text-xs border border-blue-100 whitespace-nowrap">{name.trim()}</span>))}</div></td><td className="p-4"><span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold whitespace-nowrap shadow-sm" style={{ backgroundColor: `${STATUS_COLORS[task.tienDo]}15`, color: STATUS_COLORS[task.tienDo], border: `1px solid ${STATUS_COLORS[task.tienDo]}40` }}>{getStatusIcon(task.tienDo)} {task.tienDo}</span></td><td className="p-4"><div className="flex items-center justify-center min-w-[80px]"><div className="w-full bg-gray-200 rounded-full h-1.5 md:h-2 mr-2"><div className="h-1.5 md:h-2 rounded-full" style={{ width: `${task.tyLe}%`, backgroundColor: STATUS_COLORS[task.tienDo] }}></div></div><span className="text-[10px] md:text-xs font-bold text-gray-600 w-6 text-right">{task.tyLe}%</span></div></td><td className="p-4 text-center sticky right-0 bg-white group-hover:bg-blue-50 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.05)] transition-colors"><div className="flex items-center justify-center space-x-1 md:space-x-2"><button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleOpenForm(task); }} className="p-1.5 md:p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors bg-white shadow-sm border border-gray-100"><Edit className="w-3.5 h-3.5 md:w-4 md:h-4" /></button>{canDeleteTask && (<button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteTask(task.id); }} className="p-1.5 md:p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors bg-white shadow-sm border border-gray-100"><Trash2 className="w-3.5 h-3.5 md:w-4 md:h-4" /></button>)}</div></td></tr>)) : (<tr><td colSpan={isAdmin ? "8" : "7"} className="p-8 text-center text-gray-500">Không tìm thấy công việc nào.</td></tr>)}</tbody>
                  </table>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'users' && isAdmin && (
            <div className="pb-8">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto"><table className="min-w-full text-left border-collapse"><thead><tr className="bg-gray-50 border-b border-gray-200 text-xs md:text-sm font-semibold text-gray-600"><th className="p-4 whitespace-nowrap">Họ và tên</th><th className="p-4 whitespace-nowrap">Tên đăng nhập</th><th className="p-4 whitespace-nowrap">Nhóm</th><th className="p-4 whitespace-nowrap">Quyền hạn</th><th className="p-4 text-center whitespace-nowrap">Thao tác</th></tr></thead><tbody className="divide-y divide-gray-100 text-xs md:text-sm">{usersList.map(user => (<tr key={user.id} className="hover:bg-gray-50 transition-colors"><td className="p-4 font-bold text-gray-800 whitespace-nowrap">{user.fullName}</td><td className="p-4 font-mono text-gray-500">{user.username}</td><td className="p-4 text-indigo-700 font-bold whitespace-nowrap">{user.nhom}</td><td className="p-4"><span className={`px-2.5 py-1 rounded-md text-[10px] md:text-xs font-bold border whitespace-nowrap ${user.role==='Admin' ? 'bg-rose-50 text-rose-700 border-rose-200' : user.role==='Trưởng nhóm' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>{user.role}</span></td><td className="p-4 text-center"><div className="flex items-center justify-center space-x-2"><button type="button" onClick={() => handleOpenUserForm(user)} className="p-2 bg-white border border-gray-200 text-blue-600 hover:bg-blue-50 rounded-lg shadow-sm"><Edit className="w-4 h-4" /></button><button type="button" onClick={() => handleDeleteUser(user.id)} disabled={user.id === currentUser.id} className={`p-2 bg-white border border-gray-200 rounded-lg shadow-sm ${user.id === currentUser.id ? 'text-gray-300' : 'text-red-600 hover:bg-red-50'}`}><Trash2 className="w-4 h-4" /></button></div></td></tr>))}</tbody></table></div>
            </div>
          )}
          ...
          {activeTab === 'groups' && isAdmin && (
            <div className="pb-8">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-2xl"><table className="w-full text-left border-collapse"><thead><tr className="bg-gray-50 border-b border-gray-200 text-xs md:text-sm font-semibold text-gray-600"><th className="p-4 w-16 text-center">STT</th><th className="p-4">Tên Nhóm / Phòng ban</th><th className="p-4 text-center w-24">Thao tác</th></tr></thead><tbody className="divide-y divide-gray-100 text-xs md:text-sm">{groupsList.map((group, idx) => (<tr key={group} className="hover:bg-gray-50 transition-colors"><td className="p-4 text-center text-gray-500 font-medium">{idx + 1}</td><td className="p-4 font-bold text-indigo-700">{group}</td><td className="p-4 text-center"><div className="flex items-center justify-center space-x-2"><button type="button" onClick={() => handleOpenGroupForm(group)} className="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg"><Edit className="w-4 h-4" /></button><button type="button" onClick={() => handleDeleteGroup(group)} disabled={group === 'Khác'} className={`p-1.5 rounded-lg ${group === 'Khác' ? 'text-gray-300' : 'text-red-600 hover:bg-red-100'}`}><Trash2 className="w-4 h-4" /></button></div></td></tr>))}</tbody></table></div>
            </div>
          )}
